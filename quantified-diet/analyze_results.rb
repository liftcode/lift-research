...
diet_names.each do |diet|
  ...
  puts [diet,
        ...
        experiment_for_diet.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0}.size / experiment_for_diet.find_all{|a| a["week_4_loss"]}.size.to_f].join(",")
end
...
puts ["Success rate by week for experiment ",
        ...
        experiment.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0 && a["weight_lost_percentage_day28"] && a["weight_lost_percentage_day7"] && a["weight_lost_percentage_day14"] && a["weight_lost_percentage_day21"]}.size / people_who_completed_week4_count.to_f].join(",  ")

puts ["Success rate by week for observation",
        ...
        observation.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0 && a["weight_lost_percentage_day28"] && a["weight_lost_percentage_day7"] && a["weight_lost_percentage_day14"] && a["weight_lost_percentage_day21"]}.size / observation_people_who_completed_week4_count.to_f].join(",  ")
...
puts "\n\n\nAverage weight loss by week by diet"
# diet, week 1, week 2, week3, week4
diet_names.each do |diet|
  ...
  puts [diet,
        ...
        experiment_for_diet.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0}.inject(0){|sum,a| sum += a["week_4_loss"].to_f} / experiment_for_diet.find_all{|a| a["week_4_loss"]}.size.to_f].join(",  ")
end
...
puts ["Total",
        ...
        experiment.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0 && a["weight_lost_percentage_day28"] && a["weight_lost_percentage_day7"] && a["weight_lost_percentage_day14"] && a["weight_lost_percentage_day21"]}.inject(0.0){|sum,a| sum += a["week_4_loss"].to_f} / people_who_completed_week4_count.to_f].join(",  ")
...
["0", "1"].each do |sex|
  ...
  puts [{"0" => "female", "1" => "male"}[sex],
        ...
        experiment_for_sex.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0}.inject(0){|sum,a| sum += a["week_4_loss"].to_f} / experiment_for_sex.find_all{|a| a["week_4_loss"]}.size.to_f].join(",  ")
end
...
%q{diet_include_meat_day0,diet_include_dairy_day0,diet_include_eggs_day0,diet_include_grains_day0,diet_include_candy_day0,diet_incude_soda_day0,diet_include_vegetables_day0,diet_includes_homecooked_day0,diet_include_ffood_day0}.split(",").each do |start|
  ...
  puts [start,
        ...
        experiment_for_start.find_all{|a| a["week_4_loss"] and a["week_4_loss"] < 0}.inject(0){|sum,a| sum += a["week_4_loss"].to_f} / experiment_for_start.find_all{|a| a["week_4_loss"]}.size.to_f].join(",  ")
end
...
diet_names.each do |diet|
  experiment_for_diet = experiment.find_all{|a| a["diet_randomly_assigned_name"] == diet && a["weight_lost_percentage_day28"] && a["weight_lost_percentage_day7"] && a["weight_lost_percentage_day14"] && a["weight_lost_percentage_day21"]} 
  puts [diet,
        experiment.find_all{|a| a["diet_randomly_assigned_name"] == diet && a["weight_lost_percentage_day28"]}.size / experiment.find_all{|a| a["diet_randomly_assigned_name"] == diet}.size.to_f].join(",  ")
end
...
["diet_adherence_frequency_day28", "meat_freq_day28", "nuts_freq_day28", "pasta_freq_day28", "legumes_freq_day28", "vegetables_freq_day28", "fruits_freq_day28", "dairy_freq_day28", "eggs_freq_day28", "sweets_freq_day28", "processed_foods_freq_day28", "kept_track_of_food_day28", "restricted_portions_day28", "counted_calories_day28", "stopped_eating_day28", "time_spent_eating_lunch_day28"].each do |food|
  ...
  puts [ food,
         ...
         experiment_for_food.find_all{|a| a[food] == "5"}.inject(0){|sum, a| sum += a["week_4_loss"].to_f} / experiment_for_food.find_all{|a| a[food] == "5"}.count.to_f].join(",  ")
end
...
diet_names.each do |diet|
  ...
  puts [diet,
        ...
        experiment_for_diet.find_all{|a| a["would_do_diet_again_day28"] == "Yes"}.size.to_f / experiment_for_diet.size.to_f].join(",  ")
end
